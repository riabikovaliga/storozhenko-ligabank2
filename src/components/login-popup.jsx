...
import {shakeEffect} from '../util/util';
...
const LoginPopup = (props) => {
	...
	return (
		<section className="login-popup" onClick={(evt) => {
			...
			<form className="login-popup__form" onClick={(evt) => evt.stopPropagation()} onSubmit={onPopupClose}>
				...
				<fieldset className="login-popup__wrapper">
					...
					<label className="login-popup__field">
						...
						<input className="login-popup__input" name="login" type="text" value={localStorage.getItem(`login`) !== null ? localStorage.getItem(`login`) : ``} autoFocus onChange={onInputChange} onInvalid={(evt) => {shakeEffect(evt.currentTarget)}} autoComplete="username" required></input>
					</label>
					<label className="login-popup__field login-popup__field--secret">
						<span className="login-popup__input-title">Пароль</span>
						<input className="login-popup__input" name="password" type="password" value={localStorage.getItem(`password`) !== null ? localStorage.getItem(`password`) : ``} onChange={onInputChange} onInvalid={(evt) => {shakeEffect(evt.currentTarget)}} autoComplete="current-password" required></input>
